...
如果有任一操作数是对象、数值或布尔值，则先调用toString\(\)方法再应用前面的关于字符串的规则；对于undefined和null，先调用String\(\)函数再进行运算
...
* +0 - -0 = -0
* -0 - -0 = +0
* 任一操作数是字符串、布尔值、null或undefined，先使用Number\(\)再执行运算
* 任一操作数是对象，调用valueOf\(\)再运算。如果对象没有valueOf\(\)方法，则调用toString\(\)再运算
...
* 操作数都是数值，执行数值比较
* 操作数都是字符串，逐个比较对应字符的编码
* 任一操作数是数值，则将另一个操作数转换为数值
* 任一操作数是对象，调用valueOf\(\)方法，如果没有，则调用toString\(\)方法
* 任一操作数是布尔值，转换为数值再执行比较
* 任何关系操作符在涉及NaN时会返回false

### 3.5.8 相等操作符

ECMAScript提供了两组操作符，第一组是等于和不等于，在比较之前执行转换，第二组是全等和不全等，在比较之前不执行转换
...
* 任一操作数是布尔值，转换为数值再比较
* 一个操作数是字符串，另一个操作数是数值，尝试将字符串转换为数值再比较
* 一个操作数是对象，另一个不是，则调用对象的valueOf\(\)再比较
* null == undefined
* null和undefined不能转换为其他类型的值比较
* 任一操作数是NaN，相等操作符返回false，不相等操作符返回true，且NaN不等于NaN
* 两个操作数都是对象，则比较是不是同一个对象，只有两个操所属都指向同一个对象才返回true
...
condition可以是任何表达式，ECMAScript会自动对其调用Boolean\(\)

### 3.6.2 do-while语句

一种后测试循环语句，循环体内的代码至少执行一次，语法如下：
...
for-in语句是一种严格的迭代语句，用于枚举对象中的非符号键属性，语法和例子如下：
...
ECMAScript中对象的属性无序，因此for-in语句不能保证返回对象属性的顺序。

若for-in循环迭代的变量是null或undefined，则不执行循环体

### 3.6.6 for-of语句

用于遍历可迭代对象的元素，语法和示例如下：
...
可通过break或continue引用，典型应用场景是嵌套循环

### 3.6.8 break和continue语句

break语句用于立即退出循环，强制执行循环后的下一条语句；continue语句用于立即退出循环，但会再次从循环顶部开始执行

break和continue可以与标签语句一起使用，返回代码中特定的位置，示例如下：
...
let host Name = location.hostname;
...
> switch语句在比较每个条件时使用的是全等操作符，因此不会强制转换数据类型
...
只要碰到return语句，函数就会立即停止执行并退出，所以return语句后面的代码不会被执行

return语句可以不带返回值，函数就会返回undefined，用于提前终止函数执行
